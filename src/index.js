export default {
  async fetch(request, env, ctx) {
    ...
    // ---- АДМИН-ПАНЕЛЬ /admin (единственная) ----
    if (path === '/admin') {
      const cookies = request.headers.get('Cookie') || '';
      const hasSession = cookies.includes('admin_session=prismvpn22');
      ...
      if (method === 'POST') {
        ...
        if (password === 'prismvpn22') {
          return new Response(getAdminPanel(subscriptions, url), {
            headers: {
              "Content-Type": "text/html; charset=utf-8",
              "Set-Cookie": "admin_session=prismvpn22; Max-Age=86400; Path=/; Secure; HttpOnly; SameSite=Strict"
            }
          });
        } else {
